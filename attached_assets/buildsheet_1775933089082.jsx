...
const ENGINE_DB = {
  ls1: {
    name: "GM LS1 (5.7L Gen III)",
    bore: "3.898\"", stroke: "3.622\"", displacement: "346 ci / 5.7L",
    firingOrder: "1-8-7-2-6-5-4-3", cylinders: 8,
    specs: {
      mainBearingClearance: { min: 0.0018, max: 0.0025, unit: "in", label: "Main bearing oil clearance" },
      rodBearingClearance:  { min: 0.0019, max: 0.0025, unit: "in", label: "Rod bearing oil clearance" },
      pistonToWall:         { min: 0.0007, max: 0.0021, unit: "in", label: "Piston-to-wall clearance" },
      crankEndplay:         { min: 0.001,  max: 0.012,  unit: "in", label: "Crankshaft endplay" },
      ringGapTop:           { min: 0.010,  max: 0.020,  unit: "in", label: "Top ring end gap" },
      ringGapSecond:        { min: 0.016,  max: 0.028,  unit: "in", label: "Second ring end gap" },
      ringGapOil:           { min: 0.006,  max: 0.020,  unit: "in", label: "Oil ring end gap" },
      valveStemClearanceInt:{ min: 0.001,  max: 0.0026, unit: "in", label: "Valve stem clearance (intake)" },
      valveStemClearanceExh:{ min: 0.001,  max: 0.0026, unit: "in", label: "Valve stem clearance (exhaust)" },
      camEndplay:           { min: 0.001,  max: 0.012,  unit: "in", label: "Cam endplay" },
    },
    torque: {
  ...
  sbc350: {
    name: "GM SBC 350 (Gen I)",
    bore: "4.000\"", stroke: "3.480\"", displacement: "350 ci / 5.7L",
    firingOrder: "1-8-4-3-6-5-7-2", cylinders: 8,
    specs: {
      mainBearingClearance: { min: 0.0013, max: 0.0025, unit: "in", label: "Main bearing oil clearance" },
      rodBearingClearance:  { min: 0.0013, max: 0.0035, unit: "in", label: "Rod bearing oil clearance" },
      pistonToWall:         { min: 0.0007, max: 0.0017, unit: "in", label: "Piston-to-wall clearance" },
      crankEndplay:         { min: 0.002,  max: 0.006,  unit: "in", label: "Crankshaft endplay" },
      ringGapTop:           { min: 0.010,  max: 0.020,  unit: "in", label: "Top ring end gap" },
      ringGapSecond:        { min: 0.013,  max: 0.025,  unit: "in", label: "Second ring end gap" },
      ringGapOil:           { min: 0.015,  max: 0.055,  unit: "in", label: "Oil ring end gap" },
      valveStemClearanceInt:{ min: 0.001,  max: 0.0027, unit: "in", label: "Valve stem clearance (intake)" },
      valveStemClearanceExh:{ min: 0.001,  max: 0.0027, unit: "in", label: "Valve stem clearance (exhaust)" },
      camEndplay:           { min: 0.001,  max: 0.005,  unit: "in", label: "Cam endplay" },
    },
    torque: {
  ...
  coyote50: {
    name: "Ford Coyote 5.0 (Gen II)",
    bore: "3.630\"", stroke: "3.650\"", displacement: "302 ci / 5.0L",
    firingOrder: "1-5-4-8-6-3-7-2", cylinders: 8,
    specs: {
      mainBearingClearance: { min: 0.0009, max: 0.0024, unit: "in", label: "Main bearing oil clearance" },
      rodBearingClearance:  { min: 0.0006, max: 0.0024, unit: "in", label: "Rod bearing oil clearance" },
      pistonToWall:         { min: 0.0004, max: 0.0008, unit: "in", label: "Piston-to-wall clearance" },
      crankEndplay:         { min: 0.002,  max: 0.012,  unit: "in", label: "Crankshaft endplay" },
      ringGapTop:           { min: 0.006,  max: 0.014,  unit: "in", label: "Top ring end gap" },
      ringGapSecond:        { min: 0.012,  max: 0.022,  unit: "in", label: "Second ring end gap" },
      ringGapOil:           { min: 0.006,  max: 0.026,  unit: "in", label: "Oil ring end gap" },
      valveStemClearanceInt:{ min: 0.0008, max: 0.0027, unit: "in", label: "Valve stem clearance (intake)" },
      valveStemClearanceExh:{ min: 0.0018, max: 0.0037, unit: "in", label: "Valve stem clearance (exhaust)" },
      camEndplay:           { min: 0.001,  max: 0.008,  unit: "in", label: "Cam endplay" },
    },
    torque: {
...
const COLLABORATORS = [
  { id: "jk", name: "Joe K.",   color: "#E85D04", initials: "JK" },
  { id: "mt", name: "Mike T.",  color: "#2563EB", initials: "MT" },
  { id: "sa", name: "Sara A.", color: "#059669", initials: "SA" },
];

// ─── HELPERS ─────────────────────────────────────────────────────────────────

function getStatus(value, spec, customSpec) {
...
function CollabAvatar({ user, size = 22 }) {
  return (
  ...
  if (perCyl) {
    ...
    return (
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(cylCount, 4)}, 1fr)`, gap: 4 }}>
...
export default function BuildSheetApp() {
  ...
  const [plan, setPlan]               = useState("shop");    // "free" | "builder" | "shop" | "enterprise"
  ...
  const specKeys = Object.keys(engine.specs);
  ...
  const progress = Math.round((filledCount / specKeys.length) * 100);
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif", maxWidth: 900, margin: "0 auto", padding: "1rem 0" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {["free","builder","shop","enterprise"].map(p => (
        ...
        <div style={{ background: "#1C1C1E", padding: "16px 20px", color: "#fff" }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              ...
              <select
                value={engineKey}
                onChange={e => { setEngineKey(e.target.value); setActuals({}); setEnteredBy({}); setCustomSpecs({}); }}
                style={{
              ...
              >
                {Object.entries(ENGINE_DB).map(([k, v]) => (
        ...
        {isShopOrUp && (
          <div style={{
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              {COLLABORATORS.map(u => (
                <button key={u.id} onClick={() => setCurrentUser(u)} style={{
                  ...
                  borderColor: currentUser.id === u.id ? u.color : "#E5E7EB",
                  background: currentUser.id === u.id ? u.color + "15" : "transparent",
                  cursor: "pointer"
                }}>
                  <CollabAvatar user={u} size={18} />
                  <span style={{ fontSize: 11, color: currentUser.id === u.id ? u.color : "#9CA3AF", fontWeight: 600 }}>{u.name}</span>
                </button>
              ))}
              {isEnterprise && (
                <button style={{
            ...
            <div style={{ marginLeft: "auto", fontSize: 11, color: "#9CA3AF" }}>
              Each field tracks who entered it and when
            </div>
          </div>
        )}

        {/* ── Tab Bar ── */}
        <div style={{ display: "flex", borderBottom: "0.5px solid #F3F4F6", overflowX: "auto" }}>
          {tabs.map(t => (
          ...
          {isBuilderOrUp && (
            <div style={{ marginLeft: "auto", padding: "8px 20px", display: "flex", gap: 6 }}>
              <button style={{
              ...
              {isEnterprise && (
                <div style={{
                  ...
                  <span style={{ fontSize: 12, color: "#92400E" }}>
                    <strong>Enterprise:</strong> Click "edit" next to any factory spec to override with your shop's preferred tolerances.
                    Custom specs marked with ★ and saved to your shop profile.
                  </span>
                </div>
              )}
            ...
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                {[
                  ...
                  mode === "pro" && ["Camshaft (Brand / Part #)", "camshaft"],
                  mode === "pro" && ["Timing Set (Brand / Part #)", "timingSet"],
                  mode === "pro" && ["Oil Pump (Brand / Part #)", "oilPump"],
      ...
      {editingSpec && isEnterprise && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
              {[["Your Min", "min"], ["Your Max", "max"]].map(([label, key]) => (
                <div key={key}>
                  <label style={{ fontSize: 11, color: "#6B7280", fontWeight: 600, display: "block", marginBottom: 4 }}>{label}</label>
                  <input
                    value={tempSpec[key]}
