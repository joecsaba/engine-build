...
export interface GasEngineRingRef {
  ...
  engine_name: string;
  bore_in: number;
  oem_top_min: number | null;
  oem_top_max: number | null;
  oem_second_min: number | null;
  oem_second_max: number | null;
  oem_oil_min: number | null;
  oem_oil_max: number | null;
  source: string;
  notes: string | null;
  sort_order: number;
}

export function useGasEngineRingRefs() {
  return useJsonData<GasEngineRingRef>("gas_engine_ring_refs.json");
}
