...
const HARDCODED_PLATFORMS: Record<string, PlatformDef> = {
  "12v-cummins":     { label: "5.9L 12-Valve Cummins (6BT)",     cid: 359, cylinders: 6, maxRpm: 3200, stockTurbo: "Holset HX35",            stockBoostPsi: 20, stockHp: 215, notes: "Stock P-pump governor cuts at ~2700 RPM — 3200 RPM requires a governor spring kit. Single turbo swaps are the most common mod." },
  "24v-cummins":     { label: "5.9L 24-Valve Cummins (ISB)",     cid: 359, cylinders: 6, maxRpm: 3200, stockTurbo: "Holset HY35W / HE351CW", stockBoostPsi: 24, stockHp: 325, notes: "Stock factory governor is 3200 RPM (3400+ is modified valve-float ceiling, requires upgraded springs). Common rail injection. ECU tuning + single turbo upgrade is the most popular path to 450-550 HP." },
...
const TURBO_DB: DieselTurboDef[] = [
  ...
  { name: "Holset HX40",       series: "Holset",  compressorMm: 60, minFlow: 30, maxFlow: 55, minHp: 300, maxHp: 450,  arOptions: "0.76 / 0.83 / 1.00",  flange: "T3 / T4", bearing: "journal", notes: "Most popular 12-valve upgrade. Fast spool, daily-sustainable 425-450 HP. 500 HP is achievable but requires stretched injectors + supporting pump mods.", approxPrice: "$300-700" },
  ...
  { name: "Holset HE351VE",    series: "Holset",  compressorMm: 59, minFlow: 25, maxFlow: 48, minHp: 250, maxHp: 500,  arOptions: "VGT (variable)",       flange: "T3",      bearing: "vgt",     notes: "Stock '07.5+ 6.7. Variable geometry provides excellent spool + top-end. Tune-only ceiling is 530-550 HP per community dyno data.", approxPrice: "$300-600 (used)" },
  { name: "Holset HE300VG",    series: "Holset",  compressorMm: 55, minFlow: 22, maxFlow: 45, minHp: 200, maxHp: 400,  arOptions: "VGT (variable)",       flange: "T3",      bearing: "vgt",     notes: "Stock current 6.7 (stock 55mm wheel). With 64mm aftermarket wheel upgrade, supports 550-700 HP per Diesel Power Source tiered ratings.", approxPrice: "$400-700 (used)" },
...
export default function DieselSingleTurboCalculator() {
  ...
  const PLATFORMS = useMemo(() => {
    const merged: Record<string, PlatformDef> = { ...HARDCODED_PLATFORMS };
    for (const jp of jsonDieselPlatforms) {
      if (merged[jp.slug]) continue;
      merged[jp.slug] = {
        label: jp.name,
        cid: jp.displacement_ci || 0,
        cylinders: jp.cylinders || 6,
        maxRpm: 3400,
        stockTurbo: jp.stock_turbo || "N/A",
        stockBoostPsi: 0,
