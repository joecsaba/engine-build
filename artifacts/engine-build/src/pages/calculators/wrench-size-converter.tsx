...
export default function WrenchSizeConverter() {
  const [direction, setDirection] = useUnitDirection<"sae-to-metric" | "metric-to-sae">({
  ...
  const match: Match | null = useMemo(() => {
    ...
    return {
      bestSae: s.sae,
      bestMetric: m.size,
      gapMm: direction === "sae-to-metric" ? m.gapMm : s.gapMm,
      gapIn: (direction === "sae-to-metric" ? m.gapMm : s.gapMm) / MM_PER_INCH,
      tier,
      note,
    };
  }, [direction, saePick, metricPick, customMm, customIn]);

  return (
    <div className="container mx-auto py-8 px-4 max-w-5xl">
      <SEOHead
        title="Wrench Size Converter — SAE ↔ Metric"
        description="Convert SAE (inch) wrench sizes to metric (mm) and back. Shows the closest matching wrench in the other system, the exact gap in mm and thousandths, and whether the swap is safe under torque or will round the fastener."
        canonical="/calculators/wrench-size-converter"
        keywords="wrench size converter, SAE to metric wrench, metric to SAE wrench, socket size conversion, wrench equivalent chart, 5/8 to 16mm, 3/4 to 19mm"
      />

      <h1 className="text-3xl font-bold mb-2">Wrench Size Converter — SAE ↔ Metric</h1>
      <p className="text-muted-foreground mb-8">
        Pick a wrench in one system, get the closest match in the other — plus whether the swap is actually safe under torque.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex rounded-lg border overflow-hidden">
                <button
                  className={`flex-1 py-2.5 px-4 text-sm font-medium transition-colors ${direction === "sae-to-metric" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
                  onClick={() => setDirection("sae-to-metric")}
                >
                  SAE → Metric
                </button>
                <button
                  className={`flex-1 py-2.5 px-4 text-sm font-medium transition-colors ${direction === "metric-to-sae" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
          ...
          <Card className="bg-[#1a1a1a] text-white">
            <CardHeader>
              <CardTitle>Closest {direction === "sae-to-metric" ? "Metric" : "SAE"} Wrench</CardTitle>
            </CardHeader>
            <CardContent>
              {match ? (
                <div className="space-y-4">
                ...
                <p className="text-gray-500 text-sm">Pick a wrench size or enter a value to convert.</p>
              )}
            </CardContent>
          </Card>
        </div>
