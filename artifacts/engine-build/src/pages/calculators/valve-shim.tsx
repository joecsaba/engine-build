...
/**
 * For both SOB (shim-over-bucket) and SUB (shim-under-bucket):
 *   A thicker shim makes the bucket+shim assembly taller, bringing it closer
 *   to the cam, which REDUCES (closes) the valve clearance.
 *
 *   New shim = Current shim + (Measured clearance − Desired clearance)
 *
 * If measured clearance is too large → need THICKER shim (new > current) to close gap
 * If measured clearance is too small → need THINNER shim (new < current) to open gap
 */
...
export default function ValveShimCalculator() {
  ...
  const results: ValveResult[] = useMemo(() => {
    ...
    return valves.map(v => {
      ...
      // A thicker shim closes the gap, so new clearance = measured - (newShim - currentShim)
      const resultingClearanceMm = measuredMm - (nearestShim.shimMm - currentShimMm);
      ...
      if (shimSource === "custom" && myShimsMm.length > 0 && currentShimMm > 0 && measuredMm > 0) {
        ...
        if (myMatch) {
          const myClearance = measuredMm - (myMatch.shimMm - currentShimMm);
          bestFromMyShims = {
            exact: myMatch.exact,
