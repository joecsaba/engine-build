...
const HARDCODED_TRANSMISSIONS: TransmissionDef[] = [
  {
    name: "Powerglide",
    type: "Auto",
    gears: [
...
export default function GearRatioCalculator() {
  const { data: jsonTransmissions } = useTransmissions();

  const TRANSMISSIONS = useMemo(() => {
    const merged: TransmissionDef[] = [...HARDCODED_TRANSMISSIONS];
    for (const jt of jsonTransmissions) {
      if (merged.find(m => m.name.toLowerCase() === jt.name.toLowerCase())) continue;
      // Convert JSON Transmission format to local TransmissionDef format
      const gears: { label: string; ratio: number }[] = [];
      if (jt.gear_1 != null) gears.push({ label: "1st", ratio: jt.gear_1 });
      if (jt.gear_2 != null) gears.push({ label: "2nd", ratio: jt.gear_2 });
      if (jt.gear_3 != null) gears.push({ label: "3rd", ratio: jt.gear_3 });
      if (jt.gear_4 != null) gears.push({ label: jt.speeds > 4 ? "4th" : "4th (OD)", ratio: jt.gear_4 });
      if (jt.gear_5 != null) gears.push({ label: jt.speeds > 5 ? "5th" : "5th (OD)", ratio: jt.gear_5 });
      if (jt.gear_6 != null) gears.push({ label: jt.speeds > 6 ? "6th" : "6th (OD)", ratio: jt.gear_6 });
      if (jt.gear_7 != null) gears.push({ label: "7th (OD)", ratio: jt.gear_7 });
      if (gears.length === 0) continue;
      merged.push({
        name: jt.name,
        type: jt.type === "Manual" ? "Manual" : "Auto",
        gears,
      });
    }
    return merged;
