...
const PISTON_WALL_CLEARANCE: Record<PistonMaterial, { min: number; max: number; label: string; notes: string }> = {
  "cast":           { min: 0.0010, max: 0.0020, label: "Cast aluminum", notes: "Stock replacement \u2014 tight clearance, low thermal expansion" },
  "hypereutectic":  { min: 0.0008, max: 0.0015, label: "Hypereutectic", notes: "Tightest fit of any piston type. High silicon content (16-19%) minimizes thermal expansion. Quiet on cold start." },
  "forged-4032":    { min: 0.0020, max: 0.0030, label: "Forged 4032", notes: "Moderate expansion. Good street/strip choice \u2014 quieter cold start than 2618." },
  "forged-2618":    { min: 0.0035, max: 0.0050, label: "Forged 2618", notes: "Highest expansion \u2014 noisy on cold start until piston expands to fill bore. Required for high-boost and nitrous applications." },
  "billet":         { min: 0.0020, max: 0.0050, label: "Billet", notes: "Clearance depends on alloy \u2014 4032 billet runs tighter, 2618 billet runs looser." },
};
...
export default function QuenchDeckHeightCalculator() {
  ...
  const [pistonMaterial, setPistonMaterial] = useState<PistonMaterial>("forged-4032");
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-7xl">
      <SEOHead
        ...
        keywords="quench calculator, deck height calculator, piston to deck clearance, squish calculator, quench distance, compression ratio calculator, engine quench, piston material, piston to wall clearance, forged piston clearance, hypereutectic piston, 4032 vs 2618"
      />
      ...
      {pistonMaterial === "forged-2618" && C.quench > 0 && C.quench < 0.040 && (
        <div className="mb-4 p-4 rounded-lg border bg-yellow-50 border-yellow-200">
          <p className="font-bold text-yellow-700">Tight quench with 2618 forged pistons</p>
          <p className="text-sm text-yellow-600 mt-1">2618 alloy has high thermal expansion. At cold start, piston-to-wall clearance is loose and piston rock is greater. Allow extra quench margin (0.040"+ recommended) to account for skirt flex. Current quench: {C.quench.toFixed(3)}"</p>
        </div>
      )}

      {pistonMaterial === "hypereutectic" && C.cr > 10.5 && (
        <div className="mb-4 p-4 rounded-lg border bg-yellow-50 border-yellow-200">
          <p className="font-bold text-yellow-700">High compression with hypereutectic pistons</p>
          <p className="text-sm text-yellow-600 mt-1">Hypereutectic pistons fail by shattering (brittle fracture). At {C.cr.toFixed(1)}:1 compression, detonation tolerance is reduced. Consider forged 4032 for better safety margin. Tight quench helps &mdash; your {C.quench.toFixed(3)}" quench is {C.quench <= 0.045 ? "good" : "too loose to fully suppress knock"}.</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <Field label="Piston Material" hint={PISTON_WALL_CLEARANCE[pistonMaterial].notes}>
              <Select value={pistonMaterial} onValueChange={v => setPistonMaterial(v as PistonMaterial)}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="cast">Cast Aluminum (stock)</SelectItem>
                  <SelectItem value="hypereutectic">Hypereutectic (high-silicon)</SelectItem>
                  <SelectItem value="forged-4032">Forged 4032</SelectItem>
                  <SelectItem value="forged-2618">Forged 2618</SelectItem>
      ...
      {/* ── Piston-to-Wall Clearance Reference ── */}
      <Card className="mb-8">
        <CardHeader>
          <CardTitle className="text-lg">Piston-to-Wall Clearance Reference</CardTitle>
          <p className="text-xs text-muted-foreground mt-1">
            Recommended clearance based on your selected piston material. This is separate from quench distance &mdash; piston-to-wall is measured at the skirt, perpendicular to the wrist pin.
          </p>
        </CardHeader>
        <CardContent>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted">
                  <th className="text-left p-3">Piston Type</th>
                  <th className="text-right p-3">Min Clearance</th>
                  <th className="text-right p-3">Max Clearance</th>
                  <th className="text-left p-3 pl-4">Notes</th>
                </tr>
              </thead>
              <tbody>
                {(Object.keys(PISTON_WALL_CLEARANCE) as PistonMaterial[]).map((key) => {
                  const row = PISTON_WALL_CLEARANCE[key];
                  const isActive = key === pistonMaterial;
                  return (
                    <tr
                      key={key}
                      onClick={() => setPistonMaterial(key)}
                      className={`border-b last:border-0 cursor-pointer transition-colors ${
                        isActive
                          ? "bg-[#E85D04]/10 ring-1 ring-inset ring-[#E85D04]/30 font-medium text-foreground"
                          : "hover:bg-muted/50"
                      }`}
                    >
                      <td className="p-3">{row.label}</td>
                      <td className="p-3 text-right font-mono">{row.min.toFixed(4)}"</td>
                      <td className="p-3 text-right font-mono">{row.max.toFixed(4)}"</td>
                      <td className="p-3 pl-4 text-xs text-muted-foreground">{row.notes}</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          {b > 0 && (
            <div className="mt-4 p-3 rounded-lg bg-gray-50 border text-sm">
              <p className="font-semibold">For your {b.toFixed(3)}" bore with {PISTON_WALL_CLEARANCE[pistonMaterial].label} pistons:</p>
              <p className="text-muted-foreground mt-1">
                Finish hone the cylinders to <strong>{(b + PISTON_WALL_CLEARANCE[pistonMaterial].min).toFixed(4)}"</strong> to <strong>{(b + PISTON_WALL_CLEARANCE[pistonMaterial].max).toFixed(4)}"</strong> (piston diameter + clearance).
              </p>
            </div>
          )}
        </CardContent>
